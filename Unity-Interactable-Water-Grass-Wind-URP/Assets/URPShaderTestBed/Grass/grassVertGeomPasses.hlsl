...
#ifndef GRASS_VERT_GEOM_PASS_INCLUDED
#define GRASS_VERT_GEOM_PASS_INCLUDED
...
float2 GetBladeDimensions(float3 positionWS)
{
	...
	float randomHeightNoise = SAMPLE_TEXTURE2D_LOD(_HeightNoiseTexture, sampler_HeightNoiseTexture, positionWS.xz * _HeightTextureMult, 0).r;
...
int GetBladeSegmentUniform(float3 positionWS)
{
	float d = distance(positionWS, _WorldSpaceCameraPos);
	float t;
	Unity_InverseLerp_float_Clamp01(0.0f, _grassBladeLODMaxDistance, d, t);
	...
	float3 anchorWS, float2 dimensions, float3x3 tsToWs, float2 uv)
{
	...
	half scale = 5;
	output.NoiseValue = Unity_SimpleNoise_float_Fractal(worldPosUV, scale) + 0.1;
...
void SetupOutputVertexUniform(inout TriangleStream<GeometryOutput> triStream,
    GeometryOutput output, float3 normalWS,
    float3 anchorWS, float2 dimensions, float3x3 tsToWs, float2 uv)
{
	float3 offsetTS = float3((uv.x - 0.5f) * dimensions.x, 0, uv.y * dimensions.y);//we want 0.5 to be on the center
...
void geom_TipShrink(uint primitiveID : SV_PrimitiveID, triangle Varyings input[3], inout TriangleStream<GeometryOutput> triStream) {
...
void geom_UniformQuad(uint primitiveID : SV_PrimitiveID, triangle Varyings input[3], inout TriangleStream<GeometryOutput> triStream) {
	GeometryOutput output = (GeometryOutput)0;
	...
	//get the grass width and height using the positonWS
	float2 grassWidthAndHeight = GetBladeDimensions(positionWS);
	if(grassWidthAndHeight.y < 0.3) return;
	...
	int bladeSegments_LOD = GetBladeSegmentUniform(positionWS);
	//int bladeSign = -1;
	...
	for (int i = 0; i < bladeSegments_LOD; i++)
	{
		...
		// Append the first vertex
		SetupOutputVertexUniform(triStream, output, normalWS, positionWS, grassWidthAndHeight, transform, float2(u, v));
		
		// Append the Second vertex
		SetupOutputVertexUniform(triStream, output, normalWS,positionWS, grassWidthAndHeight, transform, float2(1-u, v));
	}

	float3x3 tipTransform = FigureTransformationForHeight(1, twistMatrix, tangentToWorld, maxBend, windAxis, offsetByCollision, offsetAxis);
	// The final tip vertex should be 2
	// Append the first vertex
	SetupOutputVertexUniform(triStream, output, normalWS, positionWS, grassWidthAndHeight, tipTransform, float2(0, 1));
		
	// Append the Second vertex
	SetupOutputVertexUniform(triStream, output, normalWS,positionWS, grassWidthAndHeight, tipTransform, float2(1, 1));
