...
float4 Unity_Dither_float4(float4 In, float2 ScreenUV)
{
    float2 uv = ScreenUV;
...
half SoftShadows(float3 screenUV, float3 positionWS, half3 viewDir, half depth)
{
    ...
    half2 jitterUV = screenUV.xy * _ScreenParams.xy * 2.5;
	half shadowAttenuation = 0;

	float loopDiv = 1.0 / SHADOW_ITERATIONS;
	half depthFrac = depth * loopDiv * 0.25;
	...
	for (uint i = 0u; i < SHADOW_ITERATIONS; ++i)
    {
        ...
        jitterTexture = Unity_Dither_float4(1, jitterUV + i * _ScreenParams.xy).xyz * 2 - 1;
	    half3 j = jitterTexture.xzy * depthFrac * i * 0.01;
