#if !defined(TERRAIN_TESSELLATION_INCLUDED)
#define TERRAIN_TESSELLATION_INCLUDED
...
Varyings domain(TessellationFactors factors, OutputPatch<Varyings, 3> patch, float3 barycentricCoordinates : SV_DomainLocation)
{
	...
	MY_DOMAIN_PROGRAM_INTERPOLATE(fogFactorAndVertexLight)
	MY_DOMAIN_PROGRAM_INTERPOLATE(viewDirectionWS)
	MY_DOMAIN_PROGRAM_INTERPOLATE(viewDirectionTS)
	...
	#if !defined(LIGHTMAP_ON)
	MY_DOMAIN_PROGRAM_INTERPOLATE(sh)
	#endif
